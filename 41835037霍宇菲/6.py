...
for stock in stock_list:
    ...
    r = pd.Series(0.0, index=range(len(data)))
    for i in range(20, len(data)):
        if data.close[i] > 0: r[i] = (data.close[i - 20] -fee1)/ (data.close[i]-fee2) - 1
    rtn.append(r)
...
for i in range(len(red_3_s)):
    for j in range(len(red_3_s[i])):
           ls.append(red_3_s[i][j]),lt.append(rtn[i][j])
a=pd.Series(ls)
b=pd.Series(lt)
corr=a.corr(b)
print(corr)
...
for i in range(1, len(cum_rtn)):
    cum_rtn[i] = cum_rtn[i - 1] + daily_rtn[len(daily_rtn) - 1 - i]
cum_rtn.plot(style='-.',title='资金曲线图',color='r',label='return rate')
